...
class PointNetfeat(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.conv1(x)) # b 64 1024
        ...
        pointfeat = x
        x = self.relu(self.conv2(x)) # b 128 1024
        x = self.conv3(x) # b 1024 1024
...
class PointNetCls(nn.Module):
    ...
    def forward(self, x):
        x, trans, trans_feat = self.feat(x) # b 1024, b 3 3, b 64 64
        x_1024 = x
        x = self.relu(self.fc1(x))
        ...
        x_256 = x
        x = self.fc3(x)
        return x, trans, trans_feat, x_1024, x_256
